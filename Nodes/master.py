...
class MasterNode(Node):
    def __init__(self):
        ...
        self.objectives = {
            0: {
                "red_block": [
                    {"location": [0.15, 0.00, 0.02], "angle": 0.0, "state": UNASSIGNED},
                    {"location": [0.15, 0.05, 0.02], "angle": 0.0, "state": UNASSIGNED},
                ],
                "blue_block": [
                    {"location": [0.15, -0.05, 0.02], "angle": 0.0, "state": UNASSIGNED},
                ],
            ...
            1: {
                "red_block": [
                    {"location": [0.15, 0.00, 0.06], "angle": 0.0, "state": UNASSIGNED},
                ],
            },
        }
        self.layer = 0
    ...
    def trigger_placement(self, arm):
        ...
        target.data = [float(obj["location"][0]),
                       float(obj["location"][1]),
                       float(obj["location"][2]),
                       float(obj["angle"])]
        self.place_target_pub[arm].publish(target)
        self.get_logger().info(f"arm_{arm}: placement triggered -> {obj['location']}")
