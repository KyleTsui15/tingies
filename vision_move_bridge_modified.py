...
class IkDemo(Node):
    def __init__(self):
        ...
        # === Modified Master Integration: placement orientation intake ===
        self.place_angle = 0.0    # final placement angle already converted into this arm's local frame
        # === End Modified Master Integration: placement orientation intake ===
    ...
    def place_callback(self, msg):
        ...
        # === Modified Master Integration: use arm-local placement orientation ===
        # master.py already converted the build-frame angle into this arm's local
        # frame. This value is ready for a wrist-orientation function if/when the
        # wrist servo mapping is added.
        self.place_angle = float(msg.data[3]) if len(msg.data) >= 4 else 0.0
        # === End Modified Master Integration: use arm-local placement orientation ===
        self.publish_state(ST_PLACING)
        self.get_logger().info(
            f'arm_{self.arm_num}: placing at {place_coord}, local angle {self.place_angle:.1f} deg'
        )

        self.move_to(place_coord)
        self.wait_motion()
        # TODO: rotate wrist/gripper to self.place_angle here once servo mapping is known.
